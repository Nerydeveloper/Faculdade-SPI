...
public class ItemPedidoService {
    ...
    public ItemPedidoModel cadastrarItemPedido(ItemPedidoDTO itemPedidoDTO) {
        ProdutoModel produto = produtoRepository.findById(itemPedidoDTO.produtoId())
                .orElseThrow(() -> new EntityNotFoundException("Produto não encontrado"));

        // Aqui, você precisará de um código de pedido, ou ajusta a lógica conforme a aplicação
        PedidoModel pedido = pedidoRepository.findById(itemPedidoDTO.pedidoID())
                .orElseThrow(() -> new EntityNotFoundException("Pedido não encontrado"));

        ItemPedidoModel itemPedido = new ItemPedidoModel();
        itemPedido.setProduto(produto);
        itemPedido.setQuantidadeItem(itemPedidoDTO.quantidade());
//        itemPedido.setPrecoUnitario(itemPedidoDTO.precoUnitario());
    ...
    public ItemPedidoModel atualizarItemPedido(Long id, ItemPedidoDTO itemPedidoDTO) {
        ...
        ProdutoModel produto = produtoRepository.findById(itemPedidoDTO.produtoId()).orElseThrow(() -> new EntityNotFoundException("Produto não encontrado"));

        PedidoModel pedido = pedidoRepository.findById(itemPedidoDTO.pedidoID()).orElseThrow(() -> new EntityNotFoundException("Pedido não encontrado"));
